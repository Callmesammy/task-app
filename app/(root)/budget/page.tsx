      ...
      <div className="w-full h-full flex">
        <div className="pt-5 grid md:grid-cols-2 lg:grid-cols-3 w-full p-3 h-[12rem] gap-3">
          ...
          {isSubmitting.map((budget) => {
            const progress = (budget.amountSpent / budget.totalbalance) * 100;

            return (
              <div
                key={budget.id}
                className="border w-full h-[12rem] flex flex-col px-3"
              >
                <div className="flex justify-between px-3 pt-3 w-full items-center">
                  <div className="flex items-center space-x-3">
                    <div className="text-3xl gap-2 p-2 h-[3rem] rounded-full items-center flex bg-purple-400">
                      {budget.title}
                    </div>
